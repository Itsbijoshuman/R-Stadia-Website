...
if(isset($_GET['uremove']))
{
  $conn = new mysqli('localhost','root','','registration');
  $remove_id = $_GET['uremove'];
  mysqli_query($conn, "DELETE FROM `all_users` WHERE username = '$remove_id'") or die('query failed');

  try
  {
    mysqli_query($conn, "DELETE FROM `customer` WHERE username = '$remove_id'") or die('query failed');
  }
  catch(Exception $e)
  {
  ...
  try
  {    
    mysqli_query($conn, "DELETE FROM `owner` WHERE username = '$remove_id'") or die('query failed');
  }
  catch(Exception $e)
  {
...
if(isset($_GET['delete_all'])){
  ...
  mysqli_query($conn, "DELETE FROM `cart` WHERE user_id = '$host_id'") or die('query failed');
  header('location:Games1.php');
}
...
<!DOCTYPE html>
  ...
  <div class="user-profile">
   ...
   // include 'login.php';
    ...
    $conn = new mysqli('localhost','root','','registration');
    $select_user = mysqli_query($conn,"select * from all_users where id='$host_id'");
    ...
    <div class="flex">
      <a href="Games1.php?logout=<?php echo $host_id; ?>" onclick="return confirm('Are You Sure you want to logout?');" class="delete-btn">logout</a>
  </div>
  </div>
...
<div class="regcontainer">
    ...
    <span id="username-error" style="color: red;"></span>
      ...
      <div class="products">
          ...
          ?>
                <form method="post" class="box">
                    ...
                    <div class="price"><?php echo $fetch_game['price'] ?> ps/ sec</div>
        ...
        <div class="products">
          ...
          ?>
                <form method="post" class="box">
                    ...
                    <input type="hidden" name="product_price" value="<?php echo $fetch_game['price'] ?>">
                    <input type="hidden" name="product_des" value="<?php echo $fetch_game['des'] ?>">
                    <a href="hostdashboard.php?aremove=<?php echo $fetch_game['id']; ?>" onclick="return confirm('Are You Sure you want to Remove ?');" class="delete-btn">Remove</a>
                </form>
      <?php
              };
            };
      ?>
        ...
        <h1 class="heading">LISTED USERS</h1>
        ...
        <table>
            ...
            <tbody>
            <?php
              $user_query = mysqli_query($conn, "SELECT * FROM `all_users`") or die('query failed');
              if(mysqli_num_rows($user_query) > 0)
              {
                  while($fetch_details = mysqli_fetch_assoc($user_query))
                  {
                    if($fetch_details['type'] != 'h')
                    {
            ?>
              <tr>
                  ...
                  <td><?php echo $fetch_details['Username']; ?></td>
                  <td><?php echo $fetch_details['type']; ?></td>
                  <td>
                  <a href="hostdashboard.php?uremove=<?php echo $fetch_details['Username']; ?>" onclick="return confirm('Are You Sure you want to Remove ?');" class="delete-btn">Remove</a>
                  </td>
              </tr>
            <?php
                    }
